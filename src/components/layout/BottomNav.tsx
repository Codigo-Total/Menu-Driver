...
const NavItem = ({ href, onClick, icon: Icon, label, isActive, badge }: NavItemProps) => {
  const content = (
    <div className="flex flex-col items-center justify-center gap-1.5 w-full h-full relative transition-all duration-500">
      ...
      <span className={cn(
        ...
        isActive ? "text-brand-yellow-600 dark:text-brand-yellow-500" : "text-slate-400 dark:text-slate-600"
      )}>
        {label}
      </span>
