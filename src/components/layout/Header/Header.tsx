...
const SettingsGroup = ({ 
  ...
  <div className={cn("flex gap-3 sm:gap-4", isDesktop ? "items-center" : "flex-col")}>
    {/* Language Selection */}
      ...
      <div className="flex bg-slate-50 dark:bg-slate-900 p-0.5 rounded-lg border border-slate-100 dark:border-slate-800">
        <button
          ...
          className={cn(
            ...
            lang === 'es' ? "bg-white dark:bg-slate-800 text-brand-yellow-600 shadow-sm" : "text-slate-400 hover:text-slate-600"
          )}
        >
          ES
        </button>
        <button
          ...
          className={cn(
            "px-3 py-1.5 rounded-md text-[10px] font-black transition-all min-w-[36px]",
            lang === 'en' ? "bg-white dark:bg-slate-800 text-brand-yellow-600 shadow-sm" : "text-slate-400 hover:text-slate-600"
          )}
        >
          EN
        </button>
      ...
      <div className="flex bg-slate-50 dark:bg-slate-900 p-0.5 rounded-lg border border-slate-100 dark:border-slate-800">
        <button
          ...
          className={cn(
            ...
            theme === 'light' ? "bg-white dark:bg-slate-800 text-brand-yellow-600 shadow-sm" : "text-slate-400 hover:text-slate-600"
          )}
        >
        ...
        <button
          ...
          className={cn(
            "p-2 rounded-md transition-all flex items-center justify-center min-w-[36px]",
            theme === 'dark' ? "bg-white dark:bg-slate-800 text-brand-yellow-600 shadow-sm" : "text-slate-400 hover:text-slate-600"
          )}
        >
    ...
    {!isDesktop && (
      <div className="pt-3 border-t border-slate-100 dark:border-slate-800 mt-1">
        ...
        >
          <ShieldCheck className="h-4 w-4 transition-transform group-hover:rotate-12" /> PANEL ADMIN
        </button>
      </div>
    )}
  </div>
        ...
        <div className="flex items-center gap-3">
           ...
           <div className="flex items-center gap-2 sm:gap-3">
             <h1 className="text-lg sm:text-xl font-display font-black text-slate-900 dark:text-white uppercase tracking-tighter shrink-0">
               <span className="hidden sm:inline">Menu</span> <span className="text-brand-yellow-500">Driver</span>
             </h1>
             {pageName && (
               <>
                 <span className="text-slate-200 dark:text-slate-800 font-thin text-2xl hidden sm:block">|</span>
        ...
        <div className="flex items-center gap-3 sm:gap-5">
          ...
          <Button 
            ...
            className={cn(
              ...
              showSettings ? "bg-brand-yellow-500 text-brand-yellow-950" : "bg-slate-100 dark:bg-slate-900 text-slate-400"
            )}
          >
            <Settings className={cn("h-5 w-5 sm:h-4.5 sm:w-4.5", showSettings && "animate-spin")} />
          </Button>
        </div>
      </div>
