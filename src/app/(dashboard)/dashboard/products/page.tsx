...
import { Plus, Search, Edit2, Trash2, AlertCircle, Package, ArrowUpRight } from "lucide-react";
    ...
    <div className="space-y-8 pb-12 transition-colors duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-8">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">{t("admin.products_subtitle")}</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        {[
          ...
          <div
            key={i}
            className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center justify-between shadow-sm"
          >
            <div>
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <div
                          className={`h-1.5 w-1.5 rounded-full ${product.stock > 10 ? "bg-green-500" : product.stock > 0 ? "bg-orange-500" : "bg-red-500"}`}
                        />
                        <span
                          className={`text-sm font-medium ${product.stock === 0 ? "text-red-500 font-bold" : "text-slate-600 dark:text-slate-400"}`}
                        >
                          {product.stock} units
                        </span>
                      </div>
